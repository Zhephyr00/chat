        ...
        <div class="sidebar">
            ...
            <div class="room-list">
                @foreach($rooms as $room)
                    <a href="?room_id={{ $room->id }}" class="list-group-item list-group-item-action d-flex justify-content-between align-items-center">
                        <div>
                            <strong>{{ $room->name }}</strong>
                            <small class="text-muted d-block">{{ ucfirst($room->type) }}</small>
                        </div>
                        <span class="badge bg-danger rounded-pill d-none" id="noti-room-{{ $room->id }}">0</span>
                    </a>
                @endforeach
            </div>
        </div>
            ...
            @if(isset($activeRoom))
                ...
                <div class="p-3 bg-light border-top chat-input-bar">
                    <form action="{{ url('/send-message') }}" method="POST" class="d-flex align-items-center gap-2" id="chatForm">
                        ...
                        <input type="text" name="message_content" class="form-control" id="messageInput" placeholder="Ketik pesan..." required autocomplete="off">
                        <button type="submit" class="btn text-white px-4" style="background-color: #006699;">Kirim</button>
                    </form>
                </div>
            @else
                <div class="my-auto text-center text-muted w-100">
